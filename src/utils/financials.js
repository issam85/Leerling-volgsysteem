...
export const calculateParentPaymentStatus = (parentId, users, payments) => {
  ...
  const parentPayments = payments.filter(p => String(p.parent_id) === String(parentId)); // Striktere vergelijking
  const totalPaid = parentPayments.reduce((sum, payment) => sum + parseFloat(payment.amount || 0), 0);
  // Zorg dat amount_due een veld is op je parent user object in de database
  const amountDue = parseFloat(parent.amount_due || 0);
